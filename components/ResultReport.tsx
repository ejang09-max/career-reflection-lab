      ...
      <main className="mx-auto max-w-3xl px-4 py-16">
        <Card>
          ...
          <CardContent className="space-y-4">
            <p className="text-sm text-slate-600">브라우저에 저장된 결과나 현재 mock 서버 결과에서 해당 리포트를 찾지 못했습니다.</p>
  ...
  return (
    <main className="mx-auto max-w-6xl space-y-8 px-4 py-8 sm:px-6 lg:px-8">
      ...
      <div className="grid gap-6 lg:grid-cols-[0.95fr_1.3fr]">
        <ResultSummaryCard type={result.primaryType} />
      ...
      <div className="grid gap-6 lg:grid-cols-2">
        <Card>
          <CardHeader><CardTitle>결과 해석</CardTitle></CardHeader>
          <CardContent className="space-y-3 text-sm leading-6 text-slate-700">
            <p>{result.primaryType.description}</p>
            {result.primaryType.strengths.map((item) => <p key={item}>• {item}</p>)}
            <div className="rounded-md bg-amber-50 p-3 text-amber-950">
              <p className="mb-1 font-semibold">살펴볼 점</p>
              {result.primaryType.cautions.map((item) => <p key={item}>• {item}</p>)}
            </div>
          </CardContent>
        </Card>
        <Card>
          <CardHeader><CardTitle>추천 실천 활동</CardTitle></CardHeader>
          <CardContent className="space-y-3">
            {result.primaryType.recommendedActivities.map((activity) => <p key={activity} className="rounded-md bg-slate-50 p-3 text-sm text-slate-700">{activity}</p>)}
          </CardContent>
        </Card>
      </div>
